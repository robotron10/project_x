...
void update_limits() {
  ...
  limit_temp_C = map( limit_temp_adc, 0, 1023, 0, 100 );                                         //  mapping für Poti Grad Celsius
  
  limit_luefter_adc = analogRead( LIMIT_LUEFTER );                                               //  Werte für den ADC Temp-Sensor <-> Lüfter
  limit_luefter_C = map( limit_luefter_adc, 0, 1023, 0, 100 );                                   //  mapping für Poti Grad Celsius
